...
const ToggleButtonsMultiple = () => {
  ...
  const handleItem = (event: React.MouseEvent<HTMLElement>, newItem: string | null) => {
    if (newItem !== null) {
      setItem(newItem);
    }
  };
  ...
  return (
    <ToggleButtonGroup value={item} onChange={handleItem} exclusive aria-label="text formatting">
      <ToggleButton value="atom" aria-label="bold">
        <StopIcon />
      </ToggleButton>
      <ToggleButton value="other" aria-label="italic">
        <SearchIcon />
      </ToggleButton>
    </ToggleButtonGroup>
  );
...
const StructureView = () => {
  ...
  const ignoreLabelClick = (event: MouseEvent<Element>) => {
    event.preventDefault();
  }

  const ignoreSelect = (event: ChangeEvent<{}>, nodeIds: string[]) => {
    event.preventDefault();
  }
    ...
    <Box display="flex" flexDirection="column">
      ...
      >
        <TreeItem nodeId="1" onLabelClick={ignoreLabelClick} label={
          ...
          />
          <TreeItem nodeId="1-add" onLabelClick={ignoreLabelClick} label={
            <IconButton>
              <AddCircleIcon />
            </IconButton>
          }
